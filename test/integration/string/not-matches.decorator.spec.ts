...
import type { NOT_MATCHES, NotMatches } from '../../../src'
...
describe('@NotMatches', () => {
    ...
    const matrix: Record<string, Options[]> = {
        ...
        'each value in property must not match /^foo/i regular expression': [
            [/^foo/i, { each: true }],
        ],
        'each value in property must not match ^foo regular expression': [
            ['^foo', 'i', { each: true }],
        ],
    }
