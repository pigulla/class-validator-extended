...
import { MAP_NOT_CONTAINS_KEYS, MapNotContainsKeys } from '../../../src'
...
describe('@MapNotContainsKeys', () => {
    ...
    const mockedModule = mock.module('../../../src/map/map-not-contains-keys/map-not-contains-keys.predicate.ts', {
        namedExports: {
    ...
    for (const [message, optionsList] of Object.entries(matrix)) {
        describe(`should return the error message "${message}"`, () => {
            ...
            itEach<[Options]>(optionsList.map(item => [item]))('when called with options %j', options => {
                ...
                assert.deepEqual(mockedMapNotContainsKeys.mock.calls[0].arguments, [value, options[0]])
            })
        })
    }
})
