...
import type { IS_NETWORK_PORT, IsNetworkPort } from '../../../src'
...
describe('@IsNetworkPort', () => {
    ...
    const matrix: Record<string, Options[]> = {
        ...
        'each value in property must be a static network port': [
            [{ allow_system_allocated: false, each: true }],
        ],
        'property must be a non-system network port': [[{ allow_system_ports: false }]],
        'each value in property must be a non-system network port': [
            [{ allow_system_ports: false, each: true }],
        ],
    ...
    const mockedModule = mock.module(
        '../../../src/number/is-network-port/is-network-port.predicate.ts',
        {
            namedExports: {
